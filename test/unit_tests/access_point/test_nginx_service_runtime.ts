...
describe('[nginx] service runtime', async () => {
  ...
  it('should remove service runtimes', async() => {
    ...
    expect(result[0]).to.has.property('identity')
    expect(result[0].identity).to.has.property('application','test-application')
    expect(result[0].identity).to.has.property('name','service3')
    expect(result[0]).to.has.property('entries')
    expect(result[0].entries).to.be.instanceOf(Array)
    expect(result[0].entries.length).to.eq(2)
    expect(result[0].entries[1]).to.has.property('type','proxy')
    expect(result[0].entries[1]).to.has.property('providerInfo')
    expect(result[0].entries[1].providerInfo).to.has.property('host','nginx-reverse-proxy-server.domain')
    expect(result[0].entries[1].providerInfo).to.has.property('port',80)
    expect(result[0].entries[1].providerInfo).to.has.property('root','/server/root/some/place/on/proxy/server/1.0.3/')
    expect(result[0].entries[1].providerInfo).to.has.property('protocol','http')
